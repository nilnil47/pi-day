...
create table if not exists public.pi_reference (
...
insert into public.pi_reference (id, digits)
values (1, '')
on conflict (id) do update set digits = excluded.digits;
...
alter table public.pi_reference enable row level security;
...
create policy pi_reference_select
  on public.pi_reference for select
  to authenticated
  using (true);
...
begin
  ...
  select digits into pi_str from public.pi_reference where id = 1;
  if not found or pi_str is null or length(pi_str) < p.digits_correct + 1 then
  ...
  if digit = expected then
    ...
    update public.participants
    set eliminated = true, last_input_at = now()
    where id = p.id;
    return jsonb_build_object('ok', true, 'correct', false, 'eliminated', true, 'digits', p.digits_correct);
  end if;
end;
$$;
...
begin
  ...
  update public.participants
  set digits_correct = 0, eliminated = false, last_input_at = null
...
grant select on public.competitions, public.participants, public.pi_reference to anon, authenticated;
